...
class AzureOpenAIProvider(BaseLLMProvider):
    """LLM provider using Azure OpenAI via LangChain's AzureChatOpenAI.
    ...
    Retry behaviour
    ---------------
    ``AzureChatOpenAI`` is initialised with ``max_retries=3``, which triggers
    exponential backoff.  This satisfies the PRD requirement of 3 attempts
    with exponential backoff without introducing an extra dependency.
    ...
    def __init__(self) -> None:
        ...
        if not settings.azure_openai_deployment:
            raise ValueError(
                ...
                "Set the AZURE_OPENAI_DEPLOYMENT environment variable."
            )

        api_version = getattr(settings, "azure_openai_api_version", "2024-02-15-preview")
        temperature = getattr(settings, "azure_openai_temperature", 0.0)
        max_retries = getattr(settings, "azure_openai_max_retries", 3)
        ...
        logger.info(
            "AzureOpenAIProvider initialised (deployment=%s, endpoint=%s, max_retries=%d)",
